...
router.get('/', protect, adminOnly, async (req, res) => {
  try {
    ...
    const lowStock = inventory.filter(item => item.stockLevel <= item.lowStockThreshold).length;
  ...
  try {
    ...
    for (const update of updates) {
      try {
        ...
        if (update.lowStockThreshold !== undefined) inventory.lowStockThreshold = update.lowStockThreshold;
        if (update.reorderPoint !== undefined) inventory.reorderPoint = update.reorderPoint;
        if (update.maxStockLevel !== undefined) inventory.maxStockLevel = update.maxStockLevel;
